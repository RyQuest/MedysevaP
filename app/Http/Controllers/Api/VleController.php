...
class VleController extends Controller
{
    ...
    public function index(Request $request){
        ...
        $vle_user = VleUser::where('added_by',$user_id)->where('added_by_role','partner')->latest()->get();
        if(!empty($vle_user)){
            return response(['status' => 1,'data' => $vle_user]);
        }else{
    ...
    public function chambers(){
        $chambers = Chamber::where('name','like','NICT%')->get();
        if(!empty($chambers)){
            return response(['status' => 1,'data' => $chambers]);
        }else{
            return response(['status' => 1,'data' => '']);
        }
    }
