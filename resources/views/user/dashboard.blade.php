                 ...
                 <section class="box" style="overflow:hidden">
                        <header class="panel_header">
                            ...
                            <div class="col-md-6 mt-5">
                                <select class="form-control clinicGraphChange mt-5">
                                    ...
                                    ?>
                                    <option value="{{$monthData}}" {{date('m') == $monthData ? "selected":""}}>{{$month[$i]}}</option>
                                    @endfor
                                </select>
                            </div>
...
drawPatientGraph(patientGraphDay,patientGraph);
...
function drawPatientGraph(label,data){
    myChart1 = new Chart(myChartPatientCtx, {
    type: 'line',
    data: {
        labels: label,
        datasets: [{
...
$("body").on('change',".clinicGraphChange",function(){
    var id = $(this).val();
    $.ajax({
        "type":"get",
        "url":"/dashboard/clinic-graph",
        "data":{id:id},
        "dataType":"json",
        success:function(data){
            let dt = data.data;
            myChart1.destroy();
            // let dt1 = JSON.parse(data.patientGraphDay);
            drawPatientGraph(data.patientGraphDay,dt);
        }
    });
})
</script>
