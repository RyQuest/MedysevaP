                ...
                <div class="col-md-12">
                    <table class="table vm table-small-font no-mb table-bordered table-striped dataTable no-footer">
                        <tbody>
                            ...
                            <tr>
                                <td>Approve Date</td>
                                <td>{{date('Y-m-d H:i:s',strtotime($data->approve_date))}}</td>
                            </tr>
                            <tr>
                                <td>Bank Narration</td>
                                <td>@if($data->bank_narration) {{$data->bank_narration}}  @else -  @endif</td>
                            </tr>

                            <tr>
                                <td>Utr No</td>
                                <td>@if($data->utr_no) {{$data->utr_no}}  @else -  @endif</td>
                            </tr>
                            <tr>
                                <td>Date Of Payment</td>
                                <td>@if($data->date_of_payment) {{$data->date_of_payment}}  @else -  @endif</td>
                            </tr>
...
<div class="modal fade" id="exampleModal3" tabindex="-1" aria-labelledby="exampleModalLabel2" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
          <button type="submit" class="btn btn-secondary gradient-blue approveBtn">Submit</button>
        <button type="button" class="btn btn-secondary gradient-blue" data-dismiss="modal">Close</button>
      </div>
      </form>
    </div>
